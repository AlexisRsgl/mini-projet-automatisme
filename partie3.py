...
model = Sequential([
    Conv2D(32, (3,3), activation='relu', input_shape=(150,150,3)),  # Première convolution, 32 filtres 3x3, ReLU
    MaxPooling2D(2,2),                                              # Réduction de la taille des cartes par 2
    Conv2D(64, (3,3), activation='relu'),                            # Deuxième convolution, 64 filtres
    MaxPooling2D(2,2),                                              # Réduction par 2
    Conv2D(128, (3,3), activation='relu'),                           # Troisième convolution, 128 filtres
    MaxPooling2D(2,2),                                              # Réduction par 2
    GlobalAveragePooling2D(),                                        # Transforme chaque feature map en un seul nombre moyen
    Dense(128, activation='relu'),                                   # Couche dense pour apprendre les combinaisons de features
    Dense(1, activation='sigmoid')                                   # Couche de sortie binaire (0 = chat, 1 = chien)
])

model.compile(
    optimizer='adam',                 # Optimiseur adaptatif pour ajuster les poids
    loss='binary_crossentropy',       # Fonction de perte adaptée à la classification binaire
    metrics=['accuracy']              # On suivra la précision pendant l’entraînement
)
